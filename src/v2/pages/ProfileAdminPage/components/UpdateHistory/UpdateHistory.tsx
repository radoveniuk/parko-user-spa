...
const UpdateHistory = ({ data }: Props) => {
  ...
  const renderUpdates = (historyItem: MongoHistory<IUser>) => {
    ...
    return (Object.keys(omit(historyItem.changes, ['updatedAt', 'customFields'])) as (keyof IUser)[])
      ...
      .map((key, index) => {
        ...
        if (key === 'recruiter') {
          const recruiters: IUser[] | undefined = queryClient.getQueryData(['users', JSON.stringify({ roles: 'recruiter,admin' })]);
